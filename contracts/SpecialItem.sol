...
contract GameItems is ERC1155, ERC1155Burnable, ERC1155Supply, Ownable {
    enum Rarity { Poor, Common, Uncommon, Rare, Epic, Legendary }
    ...
    event ItemCreated(uint256 indexed itemId, string name, Rarity rarity, uint256 maxSupply);
    ...
    constructor(string memory uri) ERC1155(uri) Ownable(msg.sender) {
        for (uint256 i = 0; i < 3; i++) {
            for (uint256 j = 0; j < 6; j++) {
                _createItem(i * 6 + j, getItemName(i, Rarity(j)), Rarity(j), getMaxSupply(Rarity(j)));
            }
        }
    }

    function getItemName(uint256 itemType, Rarity rarity) internal pure returns (string memory) {
        string[3] memory itemNames = ["Sword", "Shield", "Potion"];
        string[6] memory rarityNames = ["Poor", "Common", "Uncommon", "Rare", "Epic", "Legendary"];
        return string(abi.encodePacked(rarityNames[uint(rarity)], " ", itemNames[itemType]));
    }
    ...
    function _createItem(uint256 itemId, string memory name, Rarity rarity, uint256 maxSupply) internal onlyOwner {
    ...
    function mint(address to, uint256 itemId, uint256 amount) external {
        ...
        require(totalSupply(itemId) + amount <= items[itemId].maxSupply, "Exceeds max supply");
        _mint(to, itemId, amount, "");
        emit ItemMinted(itemId, to, amount);
    }

    function burn(address from, uint256 itemId, uint256 amount) public override {
        require(msg.sender == marketplaceAddress, "Only marketplace can burn");
        _burn(from, itemId, amount);
    }

    function getItemInfo(uint256 itemId) public view returns (string memory name, Rarity rarity, uint256 maxSupply, uint256 currentSupply) {
        Item memory item = items[itemId];
        require(bytes(item.name).length > 0, "Item does not exist");
        return (item.name, item.rarity, item.maxSupply, totalSupply(itemId));
    }

    function getItemBalance(address account, uint256 itemId) public view returns (uint256) {
